...
const useHeroSlider = (items, interval = 7000) => {
  ...
  return {
    activeSlide,
    slide: items[activeSlide],
    changeSlide,
  };
};

const HeroBackground = memo(({ slide }) => (
  <>
    ...
    {/* These animations are intentionally outside the slide lifecycle.
        Changing slides does not remount or restart them. */}
    <motion.div
      animate={{
      ...
      className="absolute -left-40 top-1/4 h-[400px] w-[400px] rounded-full bg-primary/20 blur-[110px] lg:h-[500px] lg:w-[500px]"
    />
    ...
    <motion.div
      ...
      className="absolute -right-40 bottom-1/4 h-[400px] w-[400px] rounded-full bg-secondary/20 blur-[110px] lg:h-[500px] lg:w-[500px]"
    />
  </>
));

const HeroSlideContent = memo(({ slide }) => {
    ...
    <AnimatePresence mode="wait" initial={false}>
      ...
      >
        <div className="mb-3 flex items-center justify-center gap-2 lg:mb-5 lg:justify-start lg:gap-3">
          ...
          <span className="max-w-[280px] text-[9px] font-bold uppercase tracking-[0.14em] text-primary sm:text-xs sm:tracking-[0.2em]">
            {slide.eyebrow}
          </span>
        </div>
        ...
        <h1 className="text-[3rem] font-black leading-[0.9] tracking-[-0.045em] text-white sm:text-6xl md:text-7xl lg:text-8xl">
          ...
          <span
            className={
              slide.accent === "secondary" ? "text-secondary" : "text-primary"
            }
          >
            {slide.highlight}
          </span>
        ...
        {slide.type === "event" && (
          <>
            <div className="mx-auto mt-5 grid max-w-[360px] grid-cols-3 overflow-hidden rounded-2xl border border-white/10 bg-black/45 backdrop-blur-xl lg:hidden">
              ...
              <div className="min-w-0 border-x border-white/10 p-2.5 text-center">
                <Clock3 size={15} className="mx-auto mb-1 text-secondary" />
                <p className="text-[8px] font-bold uppercase tracking-wider text-white/40">
                  Time
                </p>
                <p className="mt-0.5 truncate text-[10px] font-bold text-white">
        ...
        <div className="mt-5 flex flex-col gap-2.5 sm:mt-6 sm:flex-row sm:justify-center lg:justify-start">
          <Link
            to={slide.link}
            className="btn btn-primary btn-md w-full rounded-full px-6 shadow-2xl shadow-primary/25 transition-all duration-300 hover:-translate-y-1 hover:scale-[1.02] sm:btn-lg sm:w-auto sm:px-8"
          >
            <SlideIcon size={18} />
            {slide.button}
            <ArrowRight size={17} />
          ...
          <Link
            to="/rentalStudio"
            className="btn btn-md w-full rounded-full border border-white/20 bg-white/10 px-6 text-white backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:bg-white/20 sm:btn-lg sm:w-auto sm:px-8"
          >
            <PartyPopper size={18} />
            Rent the Studio
          </Link>
...
const HeroControls = memo(({ slides, activeSlide, changeSlide }) => (
  <>
...
const HeroStableInfo = memo(() => (
  <>
    <div className="mt-4 flex flex-wrap justify-center gap-x-4 gap-y-2 text-[10px] text-white/65 sm:mt-6 sm:text-sm lg:justify-start">
      ...
      <span className="flex items-center gap-1.5">
        <MapPin size={13} className="text-secondary sm:h-[15px] sm:w-[15px]" />
        Las Vegas
      </span>
    </div>

    <div className="mx-auto mt-5 grid max-w-[360px] grid-cols-3 overflow-hidden rounded-2xl border border-white/10 bg-black/40 text-center shadow-2xl backdrop-blur-xl sm:mt-7 sm:max-w-xl lg:mx-0">
      <div className="p-2.5 sm:p-4">
        <h2 className="text-lg font-black text-primary sm:text-2xl">130+</h2>
        <p className="mt-0.5 text-[9px] text-white/50 sm:text-xs">Students</p>
      </div>

      <div className="border-x border-white/10 p-2.5 sm:p-4">
        <h2 className="text-lg font-black text-secondary sm:text-2xl">10+</h2>
      ...
      <div className="p-2.5 sm:p-4">
        <h2 className="text-lg font-black text-white sm:text-2xl">10+</h2>
        <p className="mt-0.5 text-[9px] text-white/50 sm:text-xs">Years</p>
      </div>
    </div>
  </>
));

const HeroVisual = memo(({ slide }) => (
  <div className="relative mx-auto w-full max-w-xl lg:block">
    ...
    <AnimatePresence mode="wait" initial={false}>
      <motion.div
        key={`visual-${slide.id}`}
        initial={{ opacity: 0, scale: 0.985, x: 12, filter: "blur(3px)" }}
        animate={{ opacity: 1, scale: 1, x: 0, filter: "blur(0px)" }}
        exit={{ opacity: 0, scale: 0.99, x: -8, filter: "blur(2px)" }}
        transition={{
          duration: 0.9,
          ease: [0.22, 1, 0.36, 1],
        }}
        className="relative overflow-hidden rounded-[2rem] border border-white/15 bg-white/10 p-1.5 shadow-2xl backdrop-blur-sm sm:rounded-[2.5rem] sm:p-2"
      >
        <div className="relative overflow-hidden rounded-[1.5rem] bg-black sm:rounded-[2rem]">
          <motion.img
            ...
            className={`h-[270px] w-full object-center sm:h-[430px] lg:h-[620px] ${
              slide.type === "event" ? "object-contain" : "object-cover"
            }`}
          />
              ...
              <div className="rounded-xl border border-white/15 bg-black/60 p-3 backdrop-blur-xl sm:rounded-2xl sm:p-5">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <div className="mb-1.5 inline-flex items-center gap-1.5 rounded-full bg-primary px-2 py-1 text-[8px] font-black uppercase tracking-wider text-primary-content sm:mb-2 sm:px-3 sm:text-[9px]">
                      <Sparkles size={9} />
                      Upcoming Event
                    </div>
    ...
    <AnimatePresence mode="wait" initial={false}>
      {slide.type === "event" && (
        <motion.div
          ...
          className="absolute -bottom-7 left-5 right-5 z-30 hidden rounded-2xl border border-white/15 bg-black/85 p-3 shadow-2xl backdrop-blur-xl sm:left-8 sm:right-8 sm:p-4 lg:block"
        >
    ...
    {/* Stable decorative cards: these do not depend on slide.id. */}
    <motion.div
    ...
    >
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm font-black">Host Your Class or Event</p>
...
const Hero = () => {
  const { activeSlide, slide, changeSlide } = useHeroSlider(slides);

  return (
    <section
      id="home"
      className="relative w-full overflow-hidden bg-black min-h-[100svh] lg:min-h-screen"
    >
      <HeroBackground slide={slide} />

      <div className="relative z-20 flex min-h-[100svh] items-center lg:min-h-screen">
        <div className="mx-auto w-full max-w-7xl px-4 pb-16 pt-16 sm:px-6 sm:pb-14 sm:pt-24 lg:px-10 lg:py-28">
          <div className="grid items-center gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:gap-12">
            <div className="text-center lg:text-left">
              {/* Stable brand badge. It no longer replays every 7 seconds. */}
              ...
              <div className="relative min-h-0 sm:min-h-[390px] lg:min-h-[430px]">
                <HeroSlideContent slide={slide} />
              </div>

              {/* Stable controls/info stay outside the animated slide content. */}
              <HeroControls
                slides={slides}
      ...
      <div className="absolute bottom-3 left-4 right-4 z-30 lg:hidden">
        <Link
          to="/schedule"
          className="flex w-full items-center justify-center gap-2 rounded-full border border-white/15 bg-black/45 px-4 py-2.5 text-[10px] font-bold uppercase tracking-[0.12em] text-white/70 backdrop-blur-xl"
        >
